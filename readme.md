...
vehicle file format has a size limit of 256 in all 3 directions, and placing 2
parts in the same place causes one to be deleted when loading.
...
can copy your saved vehicles to a thumb drive or USB hard drive and open them
on PC. Use a blank drive with nothing on it, formatted as FAT32 (you'll have to
reformat it on the console, but this makes sure the console will see it).
...
- Scroll past all your replays, photos, and `BANJO SAVE GAME` entries. (use the
  triggers to move 6 at a time). Each vehicle is considered a separate "save
  file".
- Click on each vehicle, then copy it to `USB Storage Device`
